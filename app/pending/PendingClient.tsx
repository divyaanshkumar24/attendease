...
export default function PendingClient({ email, rejected }: { email: string; rejected: boolean }) {
    ...
    return (
      <main className="min-h-screen bg-[#FAFAFA] dark:bg-[#0F0F0F] flex items-center justify-center px-4">
        <div className="w-full max-w-[400px] text-center">
          ...
          <button onClick={signOut} className="mt-4 text-[13px] text-[#6B6B6B] dark:text-[#A0A0A0] hover:underline">
            Sign out
          </button>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-[#FAFAFA] dark:bg-[#0F0F0F] flex items-center justify-center px-4">
      <div className="w-full max-w-[400px] text-center">
        <div className="bg-white dark:bg-[#1A1A1A] border border-[#EBEBEB] dark:border-[#2A2A2A] rounded-[10px] p-8">
          ...
          <p className="mt-4 text-[12px] text-[#ABABAB] dark:text-[#606060]">
            Signed in as <strong className="text-[#6B6B6B] dark:text-[#A0A0A0]">{email}</strong>
          </p>
        </div>
        <button onClick={signOut} className="mt-4 text-[13px] text-[#6B6B6B] dark:text-[#A0A0A0] hover:text-[#111111] dark:hover:text-[#F0F0F0] hover:underline">
          Sign out
        </button>
      </div>
    </main>
